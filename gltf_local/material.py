...
def create_image(op, idx):
    ...
    op.report({'INFO'}, 'image %d written to %s' % (idx, img.name))
...
def create_material_from_properties(op, material, material_name, use_color0):
    ...
    # create traditional texture too, cycles scuffs CATS
    op.report({'INFO'}, "mat name: %s" % material_name)
    tex = create_texture_from_material_name(op, material_name)
    slot = mat.texture_slots.add()
    slot.texture = tex
...
def create_texture_from_material_name(op, material_name):
    img_tex = None
    for img in op.gltf['images']:
        if material_name.startswith(img['name']):
            img_tex = img
            break
    ...
    op.report({'INFO'}, "%s" % img_tex)
    tex = bpy.data.textures.new(img_tex['name'], 'IMAGE')
    tex.image = tex
    return tex
