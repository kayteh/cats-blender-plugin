...
def add_in_primitive(op, bme, primitive, material_index):
    ...
    for k, target in enumerate(primitive.get('targets', [])):
        ...
        shape_name = 'Morph %d' % k

        if op.vrm:
            for bsg in op.gltf['extensions']['VRM']['blendShapeMaster']['blendShapeGroups']:
                for bind in bsg['binds']:
                    if bind['index'] == k:
                        shape_name = bsg['name']

        layer = get_layer(bme.verts.layers.shape, shape_name)
